...
const contactInfo = [];
graduates.forEach(function(chocolate) {
  contactInfo.push(`${chocolate.first_name} ${chocolate.email}`);
});
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their 
name. Create a new array called uni that contains them all. Log the result. */
const uni = [];
...
const lowerCase = zooAnimals.map(function(lowerC) {
...
*/
const largerPopulation = zooAnimals.filter(function(zanimals) {
  return zanimals.population < 5;
});
...
*/
const populationTotal = zooAnimals.reduce((total, checkmate) => {
  return total + checkmate.population;
}, 0);
